...
def after(db, Field, auth):
    ...
    db.define_table('lista',
            ...
            Field('sirka', 'decimal(6,1)', default=0.0, label=ttt('Šířka lišty (pohledová strana rámu) [cm]')),
            Field('hloubka_falcu', 'decimal(6,1)', default=0.0, label=ttt('Hloubka falcu [cm]')),
            Field('vyska', 'decimal(6,1)', default=0.0, label=ttt('Výška lišty [cm]')),
    ...
    db.define_table('lista_bv',
            ...
            Field('barva', default='', label=ttt('Barva')),
            Field('nakupni', 'decimal(8,2)', default=0.0, label=ttt('Nákupní cena'), comment=ttt('nákupní cena (lišty nebo materiálu) bez DPH [Kč/m]')),
    ...
    db.define_table('pasparta_bv',
            Field('pasparta_id', db.pasparta, writable=False, label=ttt('Typ pasparty'),
                ...
                ondelete='CASCADE'),
            Field('sada_barva_list_id', 'integer', default=1, readable = False, writable = False,
                ondelete='SET NULL'),
