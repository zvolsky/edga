...
from gluon import FORM, INPUT, URL

def ttt(txt):
    return txt  # simulace překladu

ikony = dict(ed=ttt("edit"), rm=ttt("smazat"), cp=ttt("přidat podobný"),
        fn=ttt("zvolit obrázek"), fs=ttt("ukázat přidaný náhled"),
        end=ttt("dokončení"))

def dalsi(_class='odsazenoH'):
    pojmenovane = dict(_class=_class) if _class else {}
    form = FORM(INPUT(_type='submit', _value=ttt("přidej další")), _action=URL("pridej"), **pojmenovane)
    return form

"""
# individuality edgara
